...
from sklearn.metrics import precision_recall_curve  # 添加这行导入
...
def loop_dataset_gem(classifier, sub_loader, sub_agg_loader, line_loader, line_agg_loader, optimizer=None):
    total_loss = []
    all_targets = []
    all_scores = []
    
    line_pbar = tqdm(line_loader, unit='batch')
#     line_agg_pbar = tqdm(line_agg_loader, unit='batch')
    n_samples = 0
    
    all_embeddings = []
    
    for sub_batch, sub_agg_batch, line_batch, line_agg_batch in zip(sub_loader, sub_agg_loader, line_loader, line_agg_loader):
        all_targets.extend(sub_batch.y.tolist())
        logits, loss, acc, _ = classifier(sub_batch, sub_agg_batch, line_batch, line_agg_batch)
...
#         all_scores.append(logits[:, 1].cpu().detach())
        ...
        line_pbar.set_description('loss: %0.5f acc: %0.5f' % (loss, acc))
        total_loss.append(np.array([loss, acc]) * len(sub_batch.y))
        line_pbar.update(1)
        n_samples += len(sub_batch.y)
    line_pbar.close()
    ...
    avg_loss = np.concatenate((avg_loss, [auc, aupr]))
